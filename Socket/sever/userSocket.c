...
int server_create(int no_port, unsigned int backLog)
{
    ...
    sock_sever.serv_addr.sin_addr.s_addr = inet_addr("192.168.1.15");              // auto-fill with my IP
//	sock_sever.serv_addr.sin_addr.s_addr = htonl(INADDR_ANY); 

    char str[250];
    inet_ntop(AF_INET, &(sock_sever.serv_addr.sin_addr.s_addr), str, 250 );
